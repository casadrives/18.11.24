...
export function SubscriptionPlans() {
  ...
  const handleSubscribe = async (paymentMethod: 'card' | 'paypal') => {
    try {
      // In production, this would handle the actual payment processing
      const subscriptionData = {
        email: 'user@example.com', // In production, this would be the logged-in user's email
        plan: 'Driver Premium',
        amount: 30,
      ...
      await emailService.sendSubscriptionNotification(subscriptionData);

      // Redirect to payment provider
      if (paymentMethod === 'card') {
        window.open('https://pay.sumup.com/b2c/Q34W1GM9', '_blank');
      } else {
        window.open('https://paypal.me/wissemneifer', '_blank');
      }
    } catch (error) {
